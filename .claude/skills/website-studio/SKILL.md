...
Any request to see, view, browse, get examples of, or **choose a starting
point from** what's saved - fonts, design systems, animations, or section
patterns - opens `configurator.html`, the one visual page (see **The
Website Configurator** below). Never a markdown list, never a text
description, never a table. This covers narrow requests too: "what fonts
do I have for titles" is a fonts request, not a request to see the whole
page - it still opens the Configurator, scrolled to the Fonts category,
and doesn't need a text summary of every system alongside it.
...
hasn't already) and give the URL. Never SendUserFile for this - a file
...
- **Within a session:** republish to the same file path each time (see
  `configurator.html` directly in the skill folder - no scratchpad copy
  needed, it needs no per-project substitution) so the URL stays stable
  across the whole session. First publish needs a `favicon`; omit it on
  every redeploy after.
- **In a new session,** this page has likely been published before. Before
  creating a fresh artifact, ask if a previous link exists (or check
  `action: "list"`) and republish to *that* URL with `url=` - a new
  artifact each session is exactly the file-card problem again, just
  wearing a link.

`configurator.html` is kept current between edits (see below) - a pure
"show me" request needs zero file reads first, not even the source
markdown - see **Read before working**.

**Keeping it in sync.** `configurator.html` is hand-built to match its two
markdown sources, not regenerated by a script - the content is genuinely
bespoke (real tokens per card, a real interactive demo per animation or
pattern) and doesn't reduce to a template. Whenever an entry is added to
`STUDIO-LIBRARY.md` or `PATTERN-LIBRARY.md` (`add-system`, `add these
fonts`, `save to library`, `save as a pattern`), add a matching `.cfg-card`
to `configurator.html`'s matching category in the same turn, following the
shape of the existing cards in that section. Don't let it drift - if asked
to show it and it's behind its markdown, update it first, then show it.
...
Also check for `.website-studio-selection.md` in the working directory - a
Configurator selection saved earlier this project. If present, read it now;
it changes Phase 2 and Phase 4 as described in **Reading a pasted
selection** below. Nothing to show or ask here, just read it so its picks
are already known when those phases start.
...
**Before anything else: check for a saved Configurator selection, then ask.**
If `.website-studio-selection.md` exists in the project's working directory
(see **Reading a pasted selection** below), an explicit Design System or
Fonts pick there is already the answer to (a) below - skip straight to
tuning it, no need to ask. Otherwise ask "Do you want to check your Studio
Library first?" If yes, open `configurator.html` (see **The Website
Configurator** below), scrolled to Design System / Fonts / Animations, so
the saved options can actually be seen - live colours, real font rendering,
working animation demos - not read as a markdown list. If no, go straight
to the routes below. Either way, ask once per project, not once per
sub-decision.
...
## The Website Configurator

`configurator.html` is the one visual page - built from `STUDIO-LIBRARY.md`
(source of truth for Design System, Fonts, Animations - always edit it
first) and `PATTERN-LIBRARY.md` (source of truth for every section
category), organised into one category per section: Design System, Fonts,
Hero, Testimonials / Comments, Services, About / Info, Pricing, CTA, FAQ,
Animations, Video Sections, Galleries. Every option renders its real
preview - live colour swatches and mini mockups from each design system's
real tokens, actual Google-Fonts-rendered type samples, working animation
demos (a replay button for the two that need one, a real hover state, real
clickable tabs, a running marquee), a pattern's actual code in two design
systems side by side - and is clickable. Design System, Fonts, and each
pattern category are pick-one; Animations is pick-any. Empty categories
(Services, About, Galleries today) show honestly as empty, not filled with
invented patterns - see **Pattern Library** below.

**Nothing here is required.** Clicking a card means "I already know what I
want for this one." Everything left unclicked gets decided intelligently at
build time - Design System/Fonts through Phase 2's normal routes, every
section through **Default: design it, don't assemble it** below - based on
the business, the chosen design system, and `taste.md`. The Configurator
expands what's possible; it never replaces either process.

**Finish selecting** generates a plain-text `WEBSITE-STUDIO SELECTION`
block with a Copy button, meant to be pasted straight back into the chat -
see **Reading a pasted selection** below for what happens next.

## Reading a pasted selection

A message starting with `WEBSITE-STUDIO SELECTION` is the Configurator's
output, pasted back. Write it verbatim to `.website-studio-selection.md` in
the current project's working directory (same place as
`.website-studio-learnings.md` - ask which project if none is active yet).
Confirm receipt with a one-line summary of what was picked vs. left auto.

From then on, check for this file at Phase 0 alongside the learnings check.
It changes exactly two things:

- **Phase 2 (design system).** An explicit Design System or Fonts pick is
  the project's starting point, same as picking it directly from
  `design-systems/` - still goes through the normal tuning step. Skip
  offering the three routes for whichever of the two was picked; still
  offer them for whichever was left `(auto)`.
- **Building any section** (hero, testimonials, services, about, pricing,
  cta, faq, video, galleries) **or animations.** An explicit pick for that
  category is the starting point instead of designing fresh - adapt it to
  the project's design system exactly as **Pattern Library** describes,
  same rigor, same verification. A category left `(auto)` follows
  **Default: design it, don't assemble it** as if the Configurator had
  never been used.

The file is a starting point, not a contract - normal taste corrections and
"I don't like this, show me options" (**Unhappy with what got built**
below) still apply, even to a Configurator-picked section.
...
1. **Check the Studio Library first.** Open `configurator.html`, always -
   see **The Website Configurator** above. Never skip straight to external
   sources.
...
patterns. Per **Show, don't tell** above, this is always shown in
`configurator.html`, scrolled to the matching category - every card renders
the pattern's real code twice, in two different saved design systems side
by side, proving the same layout survives a totally different look - never
the markdown, never a static screenshot.
...
2. **Unhappy with a section, wants alternatives** - "I don't like this
   hero. Show me hero options." -> open `configurator.html`, scrolled or
   linked to that category.
3. **Explicitly asked for inspiration on one section** - "give me
   inspiration for the pricing section."

On any of these three: open `configurator.html` (per **Show, don't
...
1. **Browse the Pattern Library** - open `configurator.html`, scrolled to
   that category, so the saved options can actually be seen and compared.
...
- **add this design system** / **add-system** - a website or Refero link (or
  ...
  `configurator.html`'s Design System category in the same turn (see **Show,
  don't tell**), the card's source link pointing at the same URL. It's then
  choosable for any future project like the systems that shipped with the
  library. Using it on a project never edits the original file - see
  `_adding.md`'s "Using a saved system on a project".
...
- **save to library** - a design system, font pairing or animation worth
  ...
  and a matching card to `configurator.html`'s matching category in the same
  turn. Append only, same as `taste.md` - never reorder or delete existing
  entries.
- **add these fonts / add font(s)** - one or more fonts or font
  ...
  3. Add a matching card to `configurator.html`'s Fonts category in the
     same turn (see **Show, don't tell**) - a real sample per role, not
     just the display and body tiers if there's a third (mono/accent gets
     its own small labelled sample, styled as a mono accent actually would
     be - a stat/label, not a sentence).
...
- **save as a pattern** - a section built from scratch worth reusing -> next
  ...
  card in `configurator.html`'s matching category. Same append-only rule.
  Never automatic - see **Pattern Library** above.
